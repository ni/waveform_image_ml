...
class generator:
    ...
    def gen_uniform_based_waveform_family(self, waveformsPerFamily, familyNumber):
        ...
        y = self.gen_random_uniform_based_waveform(x, segments)
        waveforms = []
    ...
    def introduce_step(self, y):
        y_error = y
        errorPointMin, errorPointMax = self.get_error_range(y)
        delta = self.get_random_value_in_y_range_with_factor(y)
        y[errorPointMin:errorPointMax] = y[errorPointMin : errorPointMax] + delta
        return y_error

    def inroduce_constant_error(self, y):
        y_error = y
        min, max = self.get_error_range(y)
        error_value = self.get_random_value_in_y_range_with_factor(y)
        y[min:max] = error_value
        return y_error

    def add_noise(self, waveforms):
