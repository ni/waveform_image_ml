...
class Generator:
    ...
    def gen_uniform_based_waveform_family(self, waveformsPerFamily, familyNumber):
        ...
        waveforms = []
        for n in tqdm(
            range(waveformsPerFamily),
            desc=f"Generating Waveforms for family {familyNumber}",
        ):
    ...
    def __get_error_range(self, y):
        ...
        return math.floor(np.min(bounds) * maxErrorLength), math.ceil(
            np.max(bounds) * maxErrorLength
        )
    ...
    def __add_noise(self, waveforms):
        scale = (np.max(waveforms) - np.min(waveforms)) * np.abs(rnd.normal(0, 0.005))
        withNoise = []
        for w in waveforms:
